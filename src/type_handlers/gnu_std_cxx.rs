...
use crate::core_dump::{CoreDump,ReadError};
...
impl CppString {
    pub fn opt_read(input: &Input, ty: &Type, addr: u64) -> Result<Option<Self>,ReadError> {
        let Type::Struct(composite_type) = ty else {
            return Ok(None);
        };
...
impl CppMapNode {
    fn read(dump: &CoreDump, addr: u64) -> Result<Self,ReadError> {
        if addr == 0 {
    ...
    pub fn next(&self, dump: &CoreDump) -> Result<Self,ReadError> {
        let mut cur_n = *self;
